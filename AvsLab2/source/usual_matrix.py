...
# Класс UsualMatrix описывает обычную квадратную матрицу как базовую альтернативу.
class UsualMatrix(Matrix):
    def __init__(self):
        super().__init__()
        self.dim = 0
        self.nums_arr = None
    ...
    def generate_matrix(self):
        ...
        :return:
        """
        self.dim = random.randint(1, 9)
        self.nums_arr = [[0.0] * self.dim] * self.dim
        for k in range(0, self.dim):
            for j in range(0, self.dim):
                self.nums_arr[k][j] = round(random.uniform(-20, 20), 5)
        pass
    ...
    def initialize(self, str_array, i):
        ...
        for k in range(0, self.dim):
            for j in range(0, self.dim):
                self.nums_arr[k][j] = float(str_array[i])
                i += 1
        return i
    ...
    def get_average(self):
        ...
        for i in range(0, self.dim):
            for j in range(0, self.dim):
                summa += self.nums_arr[i][j]
        return float(summa) / (self.dim * self.dim)

    def write(self, ostream):
        ostream.write(f"It is usual matrix. ")
